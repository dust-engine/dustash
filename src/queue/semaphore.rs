...
impl TimelineSemaphore {
    ...
    pub fn wait(self: Arc<TimelineSemaphore>, value: u64) -> impl Future<Output = VkResult<()>> {
        blocking::unblock(move || {
            ...
            info.p_semaphores = &self.semaphore;
            info.p_values = &value;
            unsafe {
                self.device.wait_semaphores(&info, std::u64::MAX)?;
            }
            drop(self);
            Ok(())
        })
    ...
    pub fn wait_n<const N: usize>(
        ...
        let device = semaphores[0].0.device.clone();
        for (semaphore, _) in semaphores.iter() {
            assert_eq!(semaphore.device.handle(), device.handle());
        }

        blocking::unblock(move || {
            let raw_semaphores: [vk::Semaphore; N] = semaphores.each_ref().map(|s| s.0.semaphore);
    ...
    pub fn wait_many(
        ...
        blocking::unblock(move || {
            let raw_semaphores: Vec<vk::Semaphore> =
                semaphores.iter().map(|s| s.0.semaphore).collect();
