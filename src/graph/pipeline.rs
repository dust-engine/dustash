...
impl PipelineCache {
    ...
    pub fn merge(&mut self, other: impl IntoIterator<Item = PipelineCache>) {
        let caches: Vec<vk::PipelineCache> = other.into_iter().map(|f| {
            assert!(Arc::ptr_eq(&self.device, &f.device));
            f.cache
        }).collect();
        unsafe {
            self.device.merge_pipeline_caches(self.cache, &caches).unwrap()
        }
    }
    pub fn merge_one(&mut self, other: PipelineCache) {
        assert!(Arc::ptr_eq(&self.device, &other.device));
        unsafe {
            self.device.merge_pipeline_caches(self.cache, &[other.cache]).unwrap()
        }
    }
    pub fn serialize(&self) -> SerializedPipelineCache {
        SerializedPipelineCache { data: unsafe {
            self.device.get_pipeline_cache_data(self.cache).unwrap().into_boxed_slice()
        } }
    }
...
impl SerializedPipelineCache {
    pub fn headers(&self) -> &vk::PipelineCacheHeaderVersionOne {
        ...
        unsafe {
            &*(slice.as_ptr() as *const vk::PipelineCacheHeaderVersionOne)
        }
    }
}
